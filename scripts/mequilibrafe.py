...
for i in range(n_ligands):
    ...
    if max_force_magnitude >= 5:
        ...
        new_directory = create_dirs(f"{ligand_work_dir}/min_2")
        ...
        edit_mdp_options(new_directory, "min_2", {"nsteps": 20000})

        new_script = write_script(new_directory, "min_2", gpu_id="2")
        new_min_sp = sp.run(["sh", f"{new_directory}/{new_script}"], capture_output=True, text=True)
        write_log_file(new_directory, ligand_number, "min_2", new_min_sp)
...
for i in range(n_ligands):
    ...
    if max_force_magnitude >= 5:
        ...
        minimised_system = bss.IO.readMolecules([f"{new_directory}/min_2.gro",
                                                f"{system_min_bound_dir}/min.top"])
        system_r_nvt_script = write_script(system_r_nvt_bound_dir, "r_nvt", gpu_id, restrained=True, previous="min_2")
